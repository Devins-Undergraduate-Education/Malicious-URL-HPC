"""
Refactored Intensive ML Model Script
...
Ensure the following packages are installed:
    - kagglehub (if additional Kaggle-related features are needed).
    - CodeCarbon (for tracking energy and CO2 metrics): pip install codecarbon
"""
...
# --- Data Visualization ---
def visualize_data(X, X_train, X_test):
    """
...
def run_lda_model(X_train, X_test, y_train, y_test, le, feature_names):
    """
...
def run_logistic_regression_model(X_train_scaled, X_test_scaled, y_train, y_test, le, feature_names):
    """
...
def run_svm_model(X_train_scaled, X_test_scaled, y_train, y_test, le, feature_names):
    """
...
def run_random_forest_model(X_train_scaled, X_test_scaled, y_train, y_test, le, feature_names):
    """
...
def main():
    ...
    Steps:
        ...
        4. Train and evaluate ML models (LDA, Logistic Regression, SVM, Random Forest).
    ...
    df, X, y, X_train, X_test, y_train, y_test, X_train_scaled, X_test_scaled, scalar, le = load_and_preprocess_data()
    
    # Data visualization
    visualize_data(X, X_train, X_test)
    
    # Model training and evaluation
    run_lda_model(X_train, X_test, y_train, y_test, le, X.columns)
    run_logistic_regression_model(X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns)
    run_svm_model(X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns)
    run_random_forest_model(X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns)
