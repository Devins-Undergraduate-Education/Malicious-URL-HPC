...
try:
    ...
    print(
        "Warning: CodeCarbon is not installed. Install it via pip for energy/CO2 tracking."
    )
...
def extract_url_features(url):
    ...
    features["has_ip_address"] = int(
        bool(re.match(r"\d+\.\d+\.\d+\.\d+", parsed.netloc))
    )
...
def run_lda_model(X_train, X_test, y_train, y_test, le, feature_names, output_folder):
    ...
    importance_df = pd.DataFrame(
        {"Feature": feature_names, "Importance": np.abs(lda.coef_).mean(axis=0)}
...
def run_logistic_regression_model(
    X_train_scaled, X_test_scaled, y_train, y_test, le, feature_names, output_folder
):
    """
    ...
    importance_df = pd.DataFrame(
        {"Feature": feature_names, "Importance": np.abs(logreg.coef_).mean(axis=0)}
    ).sort_values("Importance", ascending=False)
    fig_fi = plt.figure(figsize=(12, 6))
    sns.barplot(data=importance_df, x="Importance", y="Feature")
    ...
    duration = time.time() - start_time
    print(
        f"Logistic Regression model training and evaluation completed in {duration:.4f} seconds.\n"
    )
...
def run_svm_model(
    ...
    # Feature Importance analysis using absolute coefficients
    importance_df = pd.DataFrame(
        {"Feature": feature_names, "Importance": np.abs(svm.coef_).mean(axis=0)}
    ).sort_values("Importance", ascending=False)
    fig_fi = plt.figure(figsize=(12, 6))
    sns.barplot(data=importance_df, x="Importance", y="Feature")
...
def run_random_forest_model(
    ...
    duration = time.time() - start_time
    print(
        f"Random Forest model training and evaluation completed in {duration:.4f} seconds.\n"
    )
...
def main():
    ...
    run_logistic_regression_model(
        X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns, run_folder
    )
    run_svm_model(
        X_train_scaled, X_test_scaled, y_train, y_test, le, X.columns, run_folder
    )
